    ...
    <?php foreach ($products as $product): ?>
        <li class="lots__item lot">
            <div class="lot__image">
                <img src="<?= strip_tags($product['url']); ?>" width="350" height="260" alt="<?= strip_tags($product['title']); ?>">
            </div>
            <div class="lot__info">
                <span class="lot__category"><?= strip_tags($product['category']); ?></span>
                <h3 class="lot__title"><a class="text-link" href="lot.php?id=<?= $product['id']; ?>"><?= strip_tags($product['title']); ?></a></h3>
